...
public class BodyEditorLoader {
	...
	public void attachFixture(Body body, String name, FixtureDef fd, float scaleX, float scaleY) {
		...
		if (rbModel == null) throw new RuntimeException("Name '" + name + "' was not found in " + model.rigidBodies.size() + " elements.");
	...
	// -------------------------------------------------------------------------
	 ...
	 private Model readJson(String str) {
		...
		JsonValue bodyElem = map.get("rigidBodies");
		
		for (int i=0; i<bodyElem.size; i++) {
		     RigidBodyModel rbModel = readRigidBody(bodyElem.get(i));
	...
	private RigidBodyModel readRigidBody(JsonValue bodyElem) {
		...
		for (int i = 0; i<polygonsElem.size; i++) {
			...
			for (int j=0; j<verticesElem.size; j++) {
				JsonValue vertexElem = verticesElem.get(j);
				float x = (Float) vertexElem.get("x").asFloat();
				float y = (Float) vertexElem.get("y").asFloat();
		...
		for (int i=0; i<circlesElem.size; i++) {
			...
			circle.center.x = (Float) circleElem.get("cx").asFloat();
			circle.center.y = (Float) circleElem.get("cy").asFloat();
			circle.radius = (Float) circleElem.get("r").asFloat();
		}
